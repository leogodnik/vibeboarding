...
- Run Steps 0–8 in order. Create no files, and touch no `references/` file, before Step 8 is confirmed.
...
- «Сделать новый проект рядом» — continue the interview normally. The project goes into a subfolder named after it, created at Step 8; the files already in this folder are never touched.
...
Only after confirmation at Step 8. Fixed order, done in one pass, with no pauses for approval:
...
- If the project went into a subfolder per `## Step 8`, name that folder here too, so the user knows where to look.
