...
- **«Настоящая база данных» → PostgreSQL.** First find out whether it is already there: `pg_isready`, or one connection attempt on the default port. Running → use it, create the project's own database inside it, and say nothing about the check — the user picked this and it simply worked.
- **Chosen but not there** — never fall back in silence, and never leave the person alone with it. Step 10 promised help, so offer it in one plain sentence: it is not on this computer yet, it takes a few minutes to put there, and you will go through it together. No port numbers, no error text.
- **Then walk them through it, one step at a time.** The link and two or three steps in everyday words — macOS: `https://postgresapp.com`, download it, drag it to «Программы», open it, press **Initialize**. Windows: `https://www.postgresql.org/download/windows/`, run the installer, click through it, and write down the password it asks for. Nothing to paste into a terminal, no options they have to judge, one message per step. Wait for them to say it is done, then check again with `pg_isready` yourself — never take «готово» as proof — and if it is up, build on it and say so in one sentence.
- **The installation is their hands, not yours.** Never run it for them: no `brew install`, no downloading an installer, no starting a service on their machine. That reaches outside the project folder into the whole computer, and the person has to be the one who agrees to it and sees what happened. Your part is the link, the plain-words steps, the waiting, and the check.
- **When it does not work out** — they stop, the installer defeats them, or they would rather get going today — take it calmly and say the outcome in one plain sentence: the data goes into a file next to the project for now, and moving it over later is a settings change rather than a rebuild. Then build on SQLite and carry on. This never stops the build, and it is never an argument.
...
- Because of this, the answers that point at a real database — several sources at Step 7, several people at Step 5, «из интернета» at Step 8 — cost nothing when the user chose the file at Step 10 anyway, or when the real database did not end up on this computer. Build on SQLite and write one line into `CLAUDE.md` «Что учесть потом»: the data work is written so the base can be swapped, and moving to a real database is a settings change, not a rebuild. It is a note, not a debt. Never show a connection error, never mention a port, never present this as a compromise.
- That silence has exactly one exception, and it is the one above: when the user **chose** «настоящая база данных» at Step 10 and it was not on the computer, they are told in one plain sentence, offered the walk-through, and — if that ends in SQLite — told in one more plain sentence what they got instead. Everything they were not promised stays unsaid; the thing they picked and did not get is said.
